...
**Implementation update (2026-09-25, owner decisions: default theme, R6.1, R6.4):** The Studio now opens on the Kurva paper light theme by default; inside the VS Code Webview it follows the editor theme (`vscode-light` / `vscode-dark` / `vscode-high-contrast` body classes, observed live through a `MutationObserver`) until the user picks a theme, and a picked theme is saved under the existing `codex-avatar-studio-theme` key and wins from then on. The window-bar Settings menu gained a Theme row cycling light → dark → high contrast so dark stays one click away, and Settings → Appearance keeps direct one-click choices. The inspector page-background default tracks the theme until the user sets a custom background. `apps/studio/src/components/studioTheme.ts` and `apps/studio/test/studioTheme.test.ts` (8/8 passed) cover resolution precedence, storage failure, VS Code mapping and cycle order; Studio typecheck passed; Biome passed on the touched files. WCAG 2.x contrast computed for every text, accent, status and control-boundary pair in both themes: light 4.72:1–16.41:1, dark 5.17:1–15.12:1 — all pairs meet AA (4.5:1 text, 3:1 control boundaries). R6.4 stays unchecked: deferred by the owner until the brand work is merged and the `@kurva` npm scope is confirmed. R6.1 received its read-only preliminary availability search (table under R6.1); legal clearance stays with the owner.
...
- [ ] 22.6 Verify each IDE for real with one read and one write, and record the IDE versions.
  ...
  - BLOCKED (2026-09-25, VS Code Agent Host retry): started `code agent host --new-instance --host 127.0.0.1 --port 0 --foreground --idle-timeout 300` (ready in 135ms on `ws://localhost:50495`). `node --experimental-strip-types scripts/studio-ide-vscode-smoke.mjs` stopped immediately with “provider copilotcli has no available models.” The host was killed afterward (`code agent endpoints` empty). 22.6 stays open for VS Code authentication and WorkBuddy.
  - BLOCKED (2026-09-25, VS Code Agent Host recheck): started Agent Host 1.139.0 on `ws://localhost:58140` (ready in 107ms). `node --experimental-strip-types scripts/studio-ide-vscode-smoke.mjs` stopped before any MCP call with “provider copilotcli has no available models; authenticate or configure it before running the MCP acceptance.” Host stopped with `code agent kill`. 22.6 stays open.
- [x] 22.7 Tests:
...
- [x] 23.1 Compare every screen with its artboard at 1920, 1440, 1280, 768 and 390px in all three themes. Save the results under `.codex-avatar/previews/studio-v2/after/` and fix any overlap, clipping or weak hierarchy.
  ...
  - Evidence (2026-09-25, brand and Recents polish): side-by-side review of `screens-1440.png` showed the dark-theme Kurva mark collapsing to a vermilion curve only (stem was hardcoded `#1B1A17`), Recents cards stuck on “Loading preview…”, and the labeled Home rail collapsing below 1024px. `BrandMark` now strokes the stem with `currentColor` and the curve with `--k-vermilion`; the light-theme `filter: invert(1)` hack is removed. Recents prefer a local thumbnail immediately and only replace it after a successful host PNG load, so a missing host thumbnail shows “No preview yet” instead of a permanent loading caption. The labeled rail stays open down to 900px. Studio typecheck passed; `apps/studio/test/recentThumbnails.test.tsx` passed 1/1. After rebuilding Studio, `node scripts/studio-visual-e2e.mjs` passed (exit 0, ~40s) and refreshed the screen/state captures under `.codex-avatar/previews/studio-v2/after/`. 23.1 stays open until the full artboard side-by-side review finishes.
  - Evidence (2026-09-25, tool rail and Settings compact): side-by-side review against `docs/design/target-ui/Editor-Empty.dc.html` showed the live rail still carrying Ellipse, Line, and Arrow. The rail now matches the Target core set (Select, Hand, Frame, Rectangle, Pen, Text, Sticky note, Image or SVG, Keyboard) plus Trace image locally, which Phase 21 requires on the tool rail. Connected OpenRouter settings hide the key field until Replace key, status reads Connected, and Test / Replace / Disconnect stay on one action row so QuiverAI controls stay in the 1440×900 viewport. Studio typecheck and build passed. `node scripts/studio-visual-e2e.mjs` initially failed waiting for the Trace rail control after it was briefly removed; Trace was restored on the rail and the same script then passed (exit 0).
  - Evidence (2026-09-25, theme sync and QuiverAI honesty): visual e2e was only flipping `data-theme` without updating React theme state, so dark captures still showed the light page background `#f3eee3` in the inspector. Captures now call `window.__studioSetTheme` so tokens and page background stay in sync. When QuiverAI key storage is unavailable, Settings shows a disabled “Add key and turn on…” control with the host message instead of a dead key form. Settings keyboard shortcuts match the compact rail. `node scripts/studio-visual-e2e.mjs` passed after the theme-sync fix (exit 0); the refreshed dark 1440 editor capture shows page background `#16150f`.
  - Evidence (2026-09-25, live rebuild + connectors): Connectors visible snippets now show loopback `/mcp` only; Copy endpoint / Copy snippet inject `?studioToken=` for IDE clients. `apps/studio/test/connectorSnippets.test.ts` passed 1/1. Studio rebuild + `copy-studio-build` refreshed `apps/studio/dist` and `apps/extension/media/studio` (new hashed bundle). Live Home at the running loopback host shows Kurva BrandMark, vermilion primary actions, “Local host · saved to disk”, and Recents “No preview yet”. `node scripts/studio-visual-e2e.mjs` passed again (exit 0, ~44s). Kurva warm/paper tokens intentionally differ from Target graphite; 23.1 stays open for the remaining intentional-gap call on fixture Settings credits and missing Target-only nav.
  - Evidence (2026-09-25, Home and Settings artboard follow-up): Home mode chip now reads Build (disabled, opens in the editor) to match the Target composer. The Home rail status uses Target copy “Local host · saved to disk” when projects save locally. Warn before using a paid model is a real Settings switch (`studio-warn-paid`) that the agent panel and outbound review honor. QuiverAI’s disabled Add key and turn on… control sits above the availability message so it stays in the 1440×900 viewport. Studio typecheck and build passed; `node scripts/studio-visual-e2e.mjs` passed (exit 0). Refreshed dark 1440 Home shows Build and the local-host status; Settings shows both chat-default switches and QuiverAI’s Add key and turn on… control.
  - Evidence (2026-09-25, final layout call): refreshed Target↔Live collage at `.codex-avatar/previews/studio-v2/after/review/screens-1440-current.png` (generated by `node scripts/studio-visual-review-sheet.mjs`) pairs Home, Connectors, Settings, Editor empty, Working agent, Vector dialog, Editor light, and Mobile 390 against the captured Target references. Live matrix coverage remains complete under `.codex-avatar/previews/studio-v2/after/` (60 screen/theme/width captures plus interactive states). `node scripts/studio-visual-e2e.mjs` passed (exit 0) with overflow, frame-fit, and 44px-target assertions. Per [`docs/STUDIO_DESIGN_BRIEF.md`](STUDIO_DESIGN_BRIEF.md) and [`docs/brand/KURVA_BRAND_PLAN.md`](brand/KURVA_BRAND_PLAN.md), Kurva warm-paper / vermilion tokens restyle Target graphite colors by design and are not treated as layout defects. Intentional honesty gaps kept (not layout defects): Phase 15.2 keeps Drafts / Templates / Design systems hidden until they have real destinations; Home composer uses Build + “Choose a model in the editor” instead of a fake connected model chip; screenshot start-card copy stays local-first; Settings fixture credit reads “Not returned” rather than inventing account dollars; working-agent / vector / menu captures use deterministic fixtures so sample content differs from artboard sample data. Connectors visible snippets show loopback `/mcp` only; Copy injects the launch token. No remaining overlap, clipping, or weak hierarchy found on the reviewed screens after the earlier rail, BrandMark, Recents, Settings compact, and theme-sync fixes. 23.1 is checked.
...
- [ ] 23.8 Record per-checkbox evidence in this plan and check the §6.4 gate only after every required Studio journey passes. Keep Phase 12's separate MCP checkbox open until its live verification passes.
  - BLOCKED (2026-09-25): Do not check the §6.4 gate yet. Phase 22.6 has passing Codex, Claude Code, Qoder, and Cursor read/write probes; VS Code still needs an authenticated, ready Agent Host provider, and WorkBuddy 5.5.2 still needs its live MCP read/write probe. Phase 23.1’s layout review is now checked with intentional Kurva brand and honesty gaps recorded. The installed VSIX canvas journey still needs a valid `VITE_TLDRAW_LICENSE_KEY`. Safe next action: configure a valid canvas key for the installed VSIX pass, then reassess the remaining §6.4 checkbox. Phase 12's independent MCP check remains open.
  - Evidence (2026-09-25, partial §6.4): eight of nine §6.4 items now have recorded evidence. One remains open: installed VSIX canvas journey (tldraw key). 23.8 stays unchecked until that item closes.
...
- [ ] R6.1 Confirm the name (**Kurva**, kept by the owner in `docs/brand/KURVA_BRAND_PLAN.md`). Run a trademark search (USPTO, EUIPO, WIPO) and check domain, npm-scope and GitHub availability. **Status: preliminary search done 2026-09-25 (read-only, nothing registered or bought); legal clearance pending owner.**

  | Check | Result (2026-09-25, read-only) |
  | --- | --- |
  | `kurva.dev` | Available — RDAP returned 404 |
  | `kurva.app` | Taken — an RDAP registration record exists |
  | `kurva.design` | Available — RDAP returned 404 |
  | `getkurva.com` | Available — RDAP returned 404 |
  | npm scope `@kurva` | Likely available — registry search shows zero published packages under the scope; npmjs.com org/user pages block scripted checks (403/401), so confirm in a browser before relying on it |
  | GitHub org `kurva` | Taken — dormant personal account created 2012-01, zero public repos; an org cannot take the name while the user holds it |
  | VS Code Marketplace "Kurva" | Clear — the gallery search API returned 0 extensions |
  | USPTO (Nice 9/42) | No exact KURVA word mark found in the USPTO-data listing; the closest entries are KURV marks covering downloadable software in class 9 (one pending, others abandoned), which an examiner could treat as similar. The tmsearch API refused scripted access (405), so a final exact-match query at tmsearch.uspto.gov is still owed |
  | EUIPO / TMview | Not directly queried — the TMview API connection was blocked from this machine and web search surfaced no EUTM "KURVA". Verify interactively at tmview.europa.eu |
  | WIPO Global Brand DB | Not directly queried — same network limit. Verify at branddb.wipo.int |
  | EU morality-refusal risk | "kurva" is a strong profanity in Hungarian, Czech, Slovak, Polish and Serbian/Croatian; EUIPO may refuse under Article 7(1)(f) EUTMR (public policy / accepted principles of morality). The owner accepted this risk in the brand plan; national filings in unaffected countries remain an option |
  | Other trade-name use | Kurva Scrooge Prod GmbH (Wiesbaden, software, workmanagement.de) is an active German company name; KURVA (Belgian architecture) and kurva.es (Spanish, dissolved 2020) are non-software uses |
...
- [ ] R6.4 Rename the package scope from `@codex-avatar-studio/*` to the new scope in every `package.json`, import, tsconfig, Vite and Vitest config. Then run `pnpm install` and `pnpm run ci`. **Deferred by owner (2026-09-25) until brand work is merged and the `@kurva` npm scope is confirmed.**
...
- [x] Home, editor, agent panel, Settings and Connectors match the Target UI design canvas in light, dark and high-contrast themes at narrow and wide widths.
  - Evidence (2026-09-25): Phase 23.1 final layout call. Capture matrix under `.codex-avatar/previews/studio-v2/after/` covers 1920 / 1440 / 1280 / 768 / 390 in dark, light, and contrast; `node scripts/studio-visual-e2e.mjs` passed with overflow and frame-fit checks. Side-by-side collage `.codex-avatar/previews/studio-v2/after/review/screens-1440-current.png`. Kurva tokens restyle Target graphite by design ([`docs/STUDIO_DESIGN_BRIEF.md`](STUDIO_DESIGN_BRIEF.md)). Intentional honesty gaps: Drafts / Templates / Design systems stay hidden (15.2), Home model chip stays editor-bound, Settings credit “Not returned”, fixture sample content differs from artboard samples.
- [x] `pnpm studio` runs standalone with loopback, token, Host and Origin protection; VS Code is optional.
  - Evidence (2026-09-25): Phase 17.3–17.5 and `apps/studio-server/test/hostSecurity.test.ts` require a launch token, reject foreign Host/Origin, and bind to `127.0.0.1`. `pnpm studio` prints a loopback URL with `studioToken`. VS Code opens the same host via `codexAvatar.openStudio` (17.9) and is not required for Home or project save.
- [x] A local project can be created, edited, saved, reopened and exported without simulated controls or data loss.
  - Evidence (2026-09-25): `node scripts/studio-reopen.mjs` created a project, added a rectangle, waited for Auto-saved, restarted the host on the same library, and reopened the same rectangle (18.1e). Phase 18.5 exported PNG/SVG/JSON from a live editor session. The live VS Code Webview half of 18.1e remains open without `VITE_TLDRAW_LICENSE_KEY` and does not block this standalone gate item.
- [x] Image → SVG runs locally, and the result renders as a real, selectable and exportable vector.
  - Evidence (2026-09-25): Phase 21.1–21.2 run vtracer in a worker from the Vector asset dialog; live insert produced a selectable SVG shape that exports without scripts (18.4/18.5/21 evidence). QuiverAI stays opt-in and off by default.
- [x] The user's own OpenRouter key never reaches browser storage, bundles, messages or logs.
...
- [x] Any eligible live-catalog model can be chosen per conversation. Chat streams, stops, retries and persists, and shows cost and errors honestly.
  - Evidence (2026-09-25): Phase 19.4–19.10 load `/models/user` (public fallback on 403), persist the model per conversation, stream with Stop/Retry, and print token counts or “cost was not returned.” `pnpm smoke:studio-openrouter` completed a streamed reply against a fixture provider; isolated Edge completed a live catalog refresh and streaming chat. The VS Code Webview picker half of 19.4 stays open without a tldraw key.
