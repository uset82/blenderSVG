...
export class StudioProjectStore {
  ...
  private async getDirectory(create: boolean): Promise<string> {
    ...
    try {
      workspacePath = await realpath(configuredRoot);
    } catch {
      throw new StudioProjectStoreError("workspace", "The current workspace folder is not available on this device.");
    }

    const directory = path.resolve(workspacePath, ".codex-avatar", "studio", "projects");
    ...
    for (const segment of [".codex-avatar", "studio", "projects"]) {
      ...
      if (!isPathWithin(workspacePath, canonical)) {
        throw new StudioProjectStoreError(
          "workspace",
          "Studio project storage must remain inside the current workspace."
        );
...
function validateRenameTitle(title: string): string {
  ...
  if (!trimmed || trimmed.length > 120 || /\p{Cc}/u.test(trimmed)) {
    throw new StudioProjectStoreError("invalid-title", "Use a project title of 1–120 characters without control characters.");
  }
  return trimmed;
}
