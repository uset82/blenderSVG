...
describe("composer plus menu", () => {
  it("keeps canvas and style actions off until a canvas selection exists, and skills stay unavailable", () => {
    ...
    expect(items.find((item) => item.id === "pick-skill")?.enabled).toBe(false);
  });
