...
- [ ] Detect whether the repository is empty, an existing extension, a web application, or a monorepo.
- [ ] Record the existing architecture in `docs/ARCHITECTURE.md`.
- [ ] Record the current package manager, Node version, TypeScript version, and build commands.
- [ ] Preserve the previous `docs/PLAN_CHECKLIST.md` as `docs/PLAN_CHECKLIST_LEGACY.md` before replacing it with this plan.
...
- [ ] Create `docs/PLAN_CHECKLIST.md`.
- [ ] Create `THIRD_PARTY_NOTICES.md`.
- [ ] Create `docs/LICENSING.md`.
- [ ] Verify the license of every planned code dependency.
- [ ] Verify that no upstream character assets will be copied.
- [ ] Record upstream repository URLs, license names, and exact commit SHAs.
- [ ] Decide whether AITuber OnAir will be used only as a reference or whether specific MIT-licensed modules will be adapted.
- [ ] If code is adapted, document each source file and attribution.
- [ ] Record the Phase 0 decision lock values in `docs/ARCHITECTURE.md`.
- [ ] Add an explicit rule that artwork and model licenses are reviewed separately from code licenses.
- [ ] Add `.gitignore` entries for generated previews, caches, Blender exports, and local avatar packages.
- [ ] Confirm that the current codebase still builds before modifications.

## Acceptance criteria

- [ ] Baseline build succeeds.
- [ ] Baseline tests succeed or existing failures are documented.
- [ ] License records exist.
- [ ] No unlicensed avatar assets have been added.
- [ ] Backup branch exists.
- [ ] Existing user code has not been deleted.
